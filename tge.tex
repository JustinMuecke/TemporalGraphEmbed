...
\author{Justin Mücke% <-this % stops a space
}
...
% The paper headers
\markboth{Seminar Data-Science Wintersemester 2020/21 Universität Ulm}%
{Shell \MakeLowercase{\textit{et al.}}: Seminar Data-Science UUlm}
...
As we want to embed not just graphs, but temporal graph, lets bring in the temporal aspekt. Let's imagine a camera that makes a picture of the graph everytime a certain amount of time passed, so we get our Graphes \(G_1\) after the first picture and so on. We then take all of our pictures and put them together into one construct, thus creating our temporal graph \(G = \{G_1, G_2,\ldots ,G_t\}\) consisting of many other graphs.\cite{temporalGraphs}

\subsection{Embedding}
Our goal is, as previously mentioned, to analyze those temporal graphs, but graphs as mathematical structure are rather hard to compute with. Thus we want to transform it into a more accessable structure. For that we now try to transfer the information, that is modeled by the graph, into a vectorspace.
Those Vectors have to capture the graph topology, the vertex-to-vertex relationship as well as other relevant informations.
