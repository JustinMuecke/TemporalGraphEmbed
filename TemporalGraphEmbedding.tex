...
same structure but different orientation in the plan, thus not having an alignment, as seen in figure 3.
%Besser ausführen -> Bildchen
\begin{figure}
\includegraphics[scale=0.1]{UnalignedGraphs.png}
\caption{a) graph desrcibed by first embedding. b) graph described by second embedding. The red node do not align even though the graph structure is the same.}
\end{figure}
