...
Let \(G = (V,E)\) be a temporal graph with \(G_t=\{G_{t_1}, \ldots,G_{t_T}\} \) as its evolution over the time \(T\).
...
Depending on which outcome is wanted, different Methods have to be used to optimize the outcome. For predictions on the node level like node classification and link prediction a node-level algorithm should be used. 
On the other hand, a graph-level algorithm should be used to compare graphs with each other and to find anomalies.
